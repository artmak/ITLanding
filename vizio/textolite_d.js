	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		if (ba) {
			var bb = ba.querySelector('div>div+ol li:first-child'),
				...
				bs = ba.querySelector('div>ul+div'),
				bt = bs.firstElementChild,
				bu = bt.getAttribute('data-item'),
				bv = bt.nextElementSibling,
				bw = bv.firstElementChild,
				bx = bw.nextElementSibling,
				by = bx.nextElementSibling,
				bz = by.nextElementSibling,
				...
				bj = function() {
					var ca = function(a) {
						...
						co = function(a) {
							var b = a.querySelectorAll('.textolite-block');
							for (var c = 0, d = b.length, e; c < d; c++) {
								e = b[c].nextElementSibling;
								if (e && b[c].className == e.className) {
									var f = b[c].innerHTML.replace(new RegExp('^([\\S\\s]+?)\\s*$', 'gi'), '$1'),
										g = e.innerHTML;
									b[c].innerHTML = f + g;
									var h = b[c].getAttribute('data-original-content'),
										i = e.getAttribute('data-original-content');
									if (h || i) {
										if (!h) h = f;
										else h = h.replace(new RegExp('^([\\S\\s]+?)\\s*$', 'gi'), '$1');
										if (!i) i = g;
						...
						cc = function() {
							var a = false;
							if (dk.body.innerHTML.split('</footer>').length > 1 && dk.body.innerHTML.split('</footer>')[0].replace(new RegExp('\\t|\\n', 'gi'), '').match(new RegExp('>[^<]+?<|src="[^"]+?"', 'gi')).join('') != sr) {
								a = true;
							}
						...
						ci = function() {
							var a = this,
								...
								f = function() {
									var h = dt.contentWindow.getSelection(),
										i = h.getRangeAt(0),
										j = document.createElement('var');
									i.insertNode(j);
								...
								g = function() {
									...
									if (h.length) {
										var i = dt.contentWindow.getSelection(),
											j = h[0].nextSibling,
											k = dk.createRange();
										k.setStart(j, 0);
										k.setEnd(j, 0);
						...
						cj = function(a) {
							var b = cl(a);
							if (b.length) {
								for (var c = '', d = b.length - 1; d >= 0; d--) c += bu.split('{tagname}').join(b[d].tagName.toLowerCase());
								bt.innerHTML = c;
								for (var d = 0, e = bt.children, f = e.length; d < f; d++) {
									e[d].a = b[b.length - d - 1];
									e[d].addEventListener('click', cq);
								}
								e[d - 1].className = 'textolite-active';
								e[d - 1].a = b[0];
						...
						cp = function(a) {
							bw.className = 'textolite-disable';
							bx.className = 'textolite-disable';
							by.className = 'textolite-disable';
							bz.className = 'textolite-disable';
							bw.className = '';
							bz.className = '';
							for (var b = a.previousElementSibling; b; b = b.previousElementSibling) {
								if (b.tagName.toLowerCase() != 'script') {
									bx.className = '';
									break;
								}
							}
						...
						cq = function() {
							var a = 0,
								b = bt.querySelector('.textolite-active');
							if (b) {
								b.addEventListener('click', cq);
								b.className = '';
								ce(b.a, 'textolite-focus');
								this.removeEventListener('click', cq);
								this.className = 'textolite-active';
								cd(this.a, 'textolite-focus');
								cp(this.a);
							}
						},
						cr = function() {
							var a = dk.querySelector('.textolite-focus');
							if (a) {
								var e = a.cloneNode(true),
									f = e.querySelectorAll('.textolite-block'),
								...
								for (var g = 0, h = f.length; g < h; g++) {
									...
									f[g].ondrop = function() {
										var j = dk.querySelector('.textolite-focus img');
										if (j) {
											for (var k = j; k; k = k.parentNode) {
												if (da(k, 'textolite-focus')) {
													ce(k, 'textolite-focus');
												}
											}
											event.preventDefault();
										}
									...
									f[g].ondragend = function() {
										var a = dk.querySelector('.textolite-focus .textolite-block');
										if (a) {
											cb(a);
											cc()
										}
								...
								for (var b = [], c = a.previousElementSibling; c; c = c.previousElementSibling) {
									...
									if (c.tagName.toLowerCase() != 'script') {
										...
										for (var h = 0, i = b.length; h < i; h++) {
											if (g) g.parentNode.insertBefore(b[b.length - 1 - h], g);
											else a.parentNode.appendChild(b[b.length - 1 - h]);
										}
										co(a.parentNode);
										cp(a);
										break;
						...
						dj = function(event) {
							var a = event.target,
								...
								c = dk.querySelector('.textolite-focus');
							for (var d = a; d; d = d.parentNode) if (d == bs || d == c) return;
							ck()
						},
						df = function() {
							cd(this, 'textolite-dragover');
							return false
						},
						dg = function() {
							ce(this, 'textolite-dragover');
							return false
						},
						dh = function(event) {
							var a = this;
							event.preventDefault();
							if (window.FormData) {
								var b = event.dataTransfer.files[0];
								if (b && a.tagName.toLowerCase() == 'img' && event.dataTransfer.types[0].indexOf('text') === -1) {
									var c = bf.getAttribute('data-post-max-size') * 1,
										d = bf.getAttribute('data-upload-max-filesize') * 1;
									if (b.size < c && b.size < d) {
										var g = bf.getAttribute('data-uploading');
										ce(a, 'textolite-dragover');
										cd(a, 'textolite-drop');
										bf.innerHTML = g + ' (0%)';
										...
										ce(a, 'textolite-dragover');
										if (c > d) bf.innerHTML = bf.getAttribute('data-size-limit') + ' (' + an(d) + ')';
										else bf.innerHTML = bf.getAttribute('data-size-limit') + ' (' + an(c) + ')';
										bf.className = 'textolite-attention';
										ae(bf)
									}
								} else {
									var u = ba.a;
									if (u && u.tagName.toLowerCase() == 'img' && a.tagName.toLowerCase() == 'img') {
										var v = u.getAttribute('src');
										u.setAttribute('src', a.getAttribute('src'));
										a.setAttribute('src', v);
										ba.a = false;
										dd(this);
										cc();
									} else if (!u && (a.tagName.toLowerCase() == 'img' || a.tagName.toLowerCase() == 'edit')) {
										bf.innerHTML = bf.getAttribute('data-uploading-complete');
										bf.className = 'textolite-success';
										bd.disabled = false;
										ae(bf)
						...
						di = function() {
							var a = ca(dk.body),
								b = dk.getElementsByTagName('img'),
								c = dk.getElementsByTagName('a'),
								l = dk.getElementsByTagName('iframe'),
							...
							for (var g = 0, h = l.length; g < h; g++) {
								var m = dk.createElement('var');
								m.className = 'textolite-iframe';
							...
							dk.addEventListener('mousedown', dj);
					...
					if (dk.body.innerHTML.split('</footer>').length > 1) var sr = dk.body.innerHTML.split('</footer>')[0].replace(new RegExp('\\t|\\n', 'gi'), '').match(new RegExp('>[^<]+?<|src="[^"]+?"', 'gi')).join('');
				},
				bk = function() {
					...
					if (ca.className == 'html') {
						var cd = function(bi) {
								bi = bi.split('</_cript').join('</script').split('<_cript').join('<script').split('&').join('&amp;').split('<').join('&lt;').split('>').join('&gt;').split('&lt;var&gt;ТЗ&lt;/var&gt;').join('<var></var>').split('\n').join('<br>').replace(new RegExp('&lt;.+?&gt;', 'gi'), function(a) {
							...
							ce = function(bi) {
								return bi.replace(new RegExp('([^>](?:</span>)*|[^>])</div>', 'gi'), '$1<br></div>').split('<br>').join('\n').split('<var></var>').join('&lt;var&gt;ТЗ&lt;/var&gt;').replace(new RegExp('<.*?>', 'gi'), '').split('&lt;').join('<').split('&gt;').join('>').split('&amp;').join('&').split('</script').join('</_cript').split('<script').join('<_cript');
							};
					} else if (ca.className == 'css') {
						var cd = function(bi) {
								bi = bi.split(';').join('!~!').split('&').join('&amp;').split('<').join('&lt;').split('>').join('&gt;').split('&lt;var&gt;ТЗ&lt;/var&gt;').join('<var></var>').split('\n').join('<br>').replace(new RegExp('[^{}]+(?={)', 'gi'), function(a) {
									return '<span class=a>' + a.split('<br>').join('</span><br><span class=a>') + '</span>';
							...
							ce = function(bi) {
								return bi.replace(new RegExp('([^>](?:</span>)*|[^>])</div>', 'gi'), '$1<br></div>').split('<br>').join('\n').split('<var></var>').join('&lt;var&gt;ТЗ&lt;/var&gt;').replace(new RegExp('<.*?>', 'gi'), '').split('&lt;').join('<').split('&gt;').join('>').split('&amp;').join('&');
							};
					} else if (ca.className == 'js') {
						var cd = function(bi) {
								bi = bi.split('&').join('&amp;').split('<').join('&lt;').split('>').join('&gt;').split('&lt;var&gt;ТЗ&lt;/var&gt;').join('<var></var>').split('\n').join('<br>').replace(new RegExp('([^a-z0-9_$<]|^)([a-z]{2,10})(?=[^a-z0-9_$>])', 'gi'), function(a, b, c) {
				...
				bp = function() {
					if (ac('textolite_check')) ab('textolite_check', bf.getAttribute('data-system-url'));
				},
				bq = function(a) {
					var b = a * 1;
					if (b > 0) {
						...
						bg[0].parentNode.parentNode.style.display = 'none';
						br();
					} else if (b == -1) {
						aa('textolite_update', a, false, bf.getAttribute('data-system-url'));
						bf.innerHTML = bf.getAttribute('data-need-update') + ' (<a>' + bf.getAttribute('data-install') + '</a>)';
				...
				br = function() {};
			if (document.documentElement.id == 'textolite-code') {
				bk();
		...
		if (ba && bb) {
			...
			var bh = function(b) {
				...
				bl = function(a) {
					...
					if (a.className == 'textolite-folder' || a.className == 'textolite-backup') {
						h.a = function() {
							h.a = function() {};
							bj.call(h);
						};
						...
						if (c && b.slice(0, c.length) == c) {
							if (a.className == 'textolite-backup' && a.nextElementSibling.firstElementChild) g.addEventListener('click', bt);
							else g.className = 'textolite-disable';
				...
				bt = function() {
					var a = this.nextElementSibling,
						...
						c = this.parentNode.parentNode,
						f = function() {
							var d = c.nextElementSibling.lastElementChild.firstElementChild.firstElementChild.firstElementChild.getAttribute('data-url'),
								e = am();
							aa('textolite_token', e);
							bc.innerHTML = bc.getAttribute('data-file-recovery');
							bc.className = 'textolite-processing';
							ae(bc);
							al('recovery=' + encodeURIComponent(d) + '&token=' + e, function() {
								aa('textolite_recovery', 1, false, bc.getAttribute('data-system-url'));
								location.href = location.href.replace(location.hash, '');
							}, function() {
								bc.className = 'textolite-error';
								bc.innerHTML = bc.getAttribute('data-recovery-error');
							}, function() {
								bc.innerHTML = bc.getAttribute('data-no-response');
								bc.className = 'textolite-attention';
							});
			...
			var bu = ac('textolite_recovery');
			if (bu) {
				ab('textolite_recovery', bc.getAttribute('data-system-url'));
				bc.innerHTML = bc.getAttribute('data-recovery-success');
				bc.className = 'textolite-success';
				ae(bc);
				var bv = bd.nextElementSibling;
				bv.style.display = 'block';
				bv.className = 'textolite-menu';
				bd.className = '';
				bh(be);
			}
		}
		...
		if (ba && bb) {
			var bc = ba.querySelector('div>ol+ul>li+li>a'),
				...
				bp = function(a) {
					...
					var c = am();
					aa('textolite_token', c);
					var d = 'token=' + c + '&settings[password]=' + a + '&settings[auth_error_limit]=' + bf[0].value + '&settings[auth_lockout_duration]=' + bf[1].value + '&settings[auth_session_reset]=' + bf[2].value + '&settings[code_redraw_delay]=' + bf[3].value + '&settings[code_undo_limit]=' + bf[4].value + '&settings[default_file]=' + bf[5].value + '&settings[recovery_points]=' + bf[6].value + '&settings[logout_to_site]=' + (bt[0].checked * 1) + '&settings[site_scripts]=' + (bt[1].checked * 1) + '&settings[site_styles]=' + (bt[2].checked * 1) + '&settings[link_replacing]=' + (bt[3].checked * 1) + '&settings[image_rewriting]=' + (bt[4].checked * 1) + '&settings[code_highlighting]=' + (bt[5].checked * 1) + '&settings[update_final]=' + (bt[6].checked * 1) + '&settings[update_beta]=' + (bt[7].checked * 1);
					...
					else if (bt[5].originalValue != bt[5].checked && document.documentElement.id == 'textolite-code') b = true;
					al(d, function() {
						if (b) {
